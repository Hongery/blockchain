...
func NewSimpleTransaction(from,to string,amount int64,bc *BlockChain,txs []*Transaction)*Transaction{
	var txInputs [] *TXInput  //{TxID [] byte，Vout int，ScriptSiq string}//2.存储Txoutput的vout里面的索引//3.用户名
	var txOutputs [] *TXOuput  //Value int64，ScriptPubKey string 一个锁定脚本(ScriptPubKey)，要花这笔钱，必须要解锁该脚本。公钥：先理解为，用户名
